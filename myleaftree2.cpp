...
object_t *find_by_number(tree_node_t *tree, key_t query_key)
{  tree_node_t *tmpn;
   if( tree->left == NULL )// makes sure the tree has nodes
     return(NULL);
    
   else
   {  tmpn = tree;

             if (tmpn->right == NULL)  //if we have reached a leaf node return that leaf
                {
                 return( (object_t *) tmpn->left);
                }
      

      
          if(query_key > (tmpn->left)->leaves)  // check if leaf is in the left subtree 
               find_by_number(tmpn->right, (query_key - tmpn->left->leaves) );
          



           else                             
                find_by_number(tmpn->left, query_key); //if not in the left subtree then in the right
...
{  tree_node_t *tmp_node;
   ...
   {  tree->left = (tree_node_t *) new_object;
     ...
     {  tree_node_t * path_stack[100]; int  path_st_p = 0;
       tree_node_t * leaf_stack[100]; int  leaf_st_p = 0;
      tmp_node = tree; 
      while( tmp_node->right != NULL )
      {   path_stack[path_st_p++] = tmp_node;
          leaf_stack[leaf_st_p++] = tmp_node;
      ...
      while( leaf_st_p > 0 )
          {  int tmp_leave, old_leave;
         tmp_node = leaf_stack[--leaf_st_p];
         tmp_node->leaves = tmp_node->left->leaves + tmp_node->right->leaves;}
...
object_t *deletion(tree_node_t *tree, key_t delete_key)// i changed to deletion because delete is a keyword
...
{ 
    ...
    for (int i = COUNT; i < space; i++) 
        printf(" "); 
    printf("%d [%d]{%d}\n", root->key, root->height,root->leaves); 
...
{  tree_node_t *searchtree;
   ...
   { if( nextop == 'i' )
     ...
     { int delkey, *delobj;
       scanf(" %d", &delkey);
       delobj = deletion( searchtree, delkey);
       if( delobj == NULL )
