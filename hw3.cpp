...
{   
  ...
  for(int i = 0; i < 30; i++) 
	...
	line.push(p1);
// set begin point to visit itself
...
/*generate all neigbours and push to queue*/
   if( (head.x1 +1)>=0 && (head.x1 +1)<30 && *(g+ (head.x1+1)*2*SIZE + (head.y1*2) +head.z1)==1 )
   {  
   	explored_edge (head.x1 ,head.y1 , head.z1, head.x1 +1 ,head.y1 , head.z1);
  ...
  if( (head.x1- 1)>=0 && (head.x1 -1)<30  &&  *(g+ (head.x1-1)*2*SIZE + (head.y1*2) +head.z1)==1 )
   ...
   if( (head.y1 +1)>=0 && (head.y1 +1)<30  && *(g+ (head.x1)*2*SIZE + (head.y1+1)*2 +head.z1)==1 )
   {
   	explored_edge (head.x1 ,head.y1 , head.z1, head.x1  ,head.y1+1 , head.z1);
   	 if (!(visited[head.x1][head.y1+1][head.z1]))
   	 {
   ...
   if( (head.z1 +1)>=0 && (head.z1 +1)<2  &&  *(g+ (head.x1)*2*SIZE + (head.y1)*2 +head.z1+1)==1)
   {
   	  if (!(visited[head.x1][head.y1][head.z1+1]))
   	  {
	   ...
	   explored_edge (head.x1 ,head.y1 , head.z1, head.x1  ,head.y1 , head.z1+1);
   }
...
if (visited[tx][ty][tz]==1)
{
     ...
     while(  !(t.x1==sx && t.y1==sy && t.z1==sz) )
      ...
      path_edge(visitedfrm[t.x1][t.y1][t.z1].x1,  visitedfrm[t.x1][t.y1][t.z1].y1, visitedfrm[t.x1][t.y1][t.z1].z1, t.x1, t.y1,  t.z1);
